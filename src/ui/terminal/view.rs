...
pub struct TerminalView {
    ...
    /// Whether the cursor should be painted for the current blink phase.
    cursor_visible: bool,
    ...
    /// Async task that drives cursor blinking.
    #[allow(dead_code)]
...
impl TerminalView {
        ...
        R: Read + Send + 'static,
    {
        ...
        let cursor_blink_task = cx.spawn(async move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
            loop {
    ...
    pub fn update_colors(&mut self, colors: ColorPalette, cx: &mut Context<Self>) {
        self.renderer.palette = colors.clone();
            ...
            .child(
                canvas(
                    ...
                    move |bounds, _, window, cx| {
                        ...
                        measured_renderer.paint(bounds, padding, &term, cursor_visible, window, cx);
                    },
                )
                .size_full(),
            )
